...
let anyMatchingCells anon raw =
  Array.zip anon raw
  |> Array.tryFind(fun (a, r) -> a = r)
  |> Option.isSome
...
let compareResults (anon: QueryResult) (raw: QueryResult) : ComparativeResult =
  let rec compareRowsRecursively a r =
    ...
    | an :: ans, r :: rs ->
      if anyMatchingCells an r
      then (Mixed <| matchUpCells an r) :: compareRowsRecursively ans rs
      else (AnonymizedAway r) :: compareRowsRecursively (an :: ans) rs
    ...
    | _, _ -> compareRowsRecursively a r
  {
    ...
    Rows = compareRows anon.Rows raw.Rows
  }
...
let update message model =
  ...
  | SetQueryResult result ->
    { model with Result = Some result; Error = None }, Cmd.none
  | SetError error ->
    { model with Error = Some error }, Cmd.none
  | AdjustAnonParam adjuster ->
    { model with AnonParams = adjuster model.AnonParams }, Cmd.ofMsg RunQuery
    ...
    |> Array.map (
      ...
      | MixedCell mixed ->
        td [Classes (cellClasses mixed.RawValue)] [
          div [Classes ["rounded-full inline border text-base bg-green-50 inline-flex items-center"]] [
            span [Classes ["bg-green-200 rounded-full pl-1 pr-1.5"]] [valueToNode mixed.AnonymizedValue]
            span [Classes ["ml-0.5 text-xs pr-1 h-full text-green-500"]] [text "A"]
          ]
          span [Classes ["text-gray-200 mx-1"]] [text "|"]
          div [Classes ["rounded-full inline border text-base bg-yellow-50 inline-flex items-center"]] [
            span [Classes ["bg-yellow-200 rounded-full pl-1 pr-1.5"]] [valueToNode mixed.RawValue]
            span [Classes ["ml-0.5 text-xs pr-1 h-full text-yellow-500"]] [text "R"]
          ]
        ]
    )
    |> Array.toList
...
let resultTable (result: ComparativeResult) =
  table [Classes ["w-full"]] [
    ...
    tbody [] (
       ...
       |> List.map (
          function
          | AnonymizedAway row -> renderRow ["bg-red-200"] row
          | Identical row -> renderRow ["bg-green-200"] row
...
let homePage model dispatch =
  Main
    ...
    .MinimumNumberAIDs(model.AnonParams.LowCountAbsoluteLowerBound,
                       fun v -> dispatch <| AdjustAnonParam (fun a -> {a with LowCountAbsoluteLowerBound = v; LowCountThreshold = {Lower = v; Upper = v+2}}))
    .OutlierMin(model.AnonParams.OutlierCount.Lower,
                fun v -> dispatch <| AdjustAnonParam (fun a -> {a with OutlierCount = {a.OutlierCount with Lower = v}}))
    .OutlierMax(model.AnonParams.OutlierCount.Upper,
                fun v -> dispatch <| AdjustAnonParam (fun a -> {a with OutlierCount = {a.OutlierCount with Upper = v}}))
    .TopMin(model.AnonParams.TopCount.Lower,
            fun v -> dispatch <| AdjustAnonParam (fun a -> {a with TopCount = {a.TopCount with Lower = v}}))
    .TopMax(model.AnonParams.TopCount.Upper,
            fun v -> dispatch <| AdjustAnonParam (fun a -> {a with TopCount = {a.TopCount with Upper = v}}))
    .NoiseStdDev(model.AnonParams.Noise.StandardDev,
                 fun v -> dispatch <| AdjustAnonParam (fun a -> {a with Noise = {a.Noise with StandardDev = v}}))
    .NoiseCutoff(model.AnonParams.Noise.Cutoff,
                 fun v -> dispatch <| AdjustAnonParam (fun a -> {a with Noise = {a.Noise with Cutoff = v}}))
