...
type private Count() =
  ...
  interface IAggregator with
    ...
    member this.Final(_aggContext, _anonContext, _outliersAggregator) = Integer state
...
type private CountDistinct() =
  ...
  interface IAggregator with
    ...
    member this.Final(_aggContext, _anonContext, _outliersAggregator) = state.Count |> int64 |> Integer
...
type private Sum() =
  ...
  interface IAggregator with
    ...
    member this.Final(_aggContext, _anonContext, _outliersAggregator) = state
...
type private DiffixCount(aidsCount) =
  ...
  interface IAggregator with
    ...
    member this.Final(aggContext, anonContext, outliersAggregator) =
      let anonContext = unwrapAnonContext anonContext
      let getState = fun aggregator -> (castAggregator<DiffixCount> aggregator).State
      ...
      | Some result ->
        Option.iter (getState >> mergeOutliers result.Outliers) outliersAggregator
...
type private DiffixSum(summandType, aidsCount) =
  ...
  interface IAggregator with
    ...
    member this.Final(aggContext, anonContext, outliersAggregator) =
      let anonContext = unwrapAnonContext anonContext
      let getPositiveState = fun aggregator -> (castAggregator<DiffixSum> aggregator).State.Positive
      let getNegativeState = fun aggregator -> (castAggregator<DiffixSum> aggregator).State.Negative
      ...
      | Some result ->
        Option.iter (getPositiveState >> mergeOutliers result.PositiveOutliers) outliersAggregator
        Option.iter (getNegativeState >> mergeOutliers result.NegativeOutliers) outliersAggregator
