...
open OpenDiffix.Parser.QueryParser
...
let ``Parses simple identifiers`` () =
  ...
  (parseFragment expr "hello.bar12") |> should equal (Identifier(Some "hello", "bar12")) // Allows qualified names
...
let ``Parses expressions`` () =
  ...
  [ "+"; "-"; "*"; "/"; "^"; "%" ]
  |> List.iter (fun op -> parseFragment expr $"1 %s{op} 1" |> should equal (Expression.Function(op, [ Integer 1; Integer 1 ])))

  [ "and", And; "or", Or; "<", Lt; "<=", LtE; ">", Gt; ">=", GtE; "=", Equals; "<>", Not << Equals ]
  |> List.iter (fun (op, expected) -> parseFragment expr $"1 %s{op} 1" |> should equal (expected (Integer 1, Integer 1)))

  parseFragment expr "not 1" |> should equal (Expression.Not(Integer 1))
  parseFragment expr "value is null" |> should equal (IsNull(Identifier(None, "value")))
  parseFragment expr "value is not null" |> should equal (Not(IsNull(Identifier(None, "value"))))

[<Fact>]
let ``Parses columns`` () =
  parseFragment commaSepExpressions "hello" |> should equal [ As(Identifier(None, "hello"), None) ]
  parseFragment commaSepExpressions "hello as alias" |> should equal [ As(Identifier(None, "hello"), Some "alias") ]
...
let ``Parses functions`` () =
  parseFragment expr "hello(world)" |> should equal (Function("hello", [ Identifier(None, "world") ]))
  parseFragment expr "hello ( world )" |> should equal (Function("hello", [ Identifier(None, "world") ]))
...
let ``Parses complex functions`` () =
  ...
  |> should equal (Function("length", [ Function("sum", [ Function("+", [ Identifier(None, "rain"); Identifier(None, "sun") ]) ]) ]))

[<Fact>]
let ``Parses WHERE clause conditions`` () =
  parseFragment whereClause "WHERE a = 1" |> should equal (Equals(Identifier(None, "a"), Integer 1))
  parseFragment whereClause "WHERE a = '1'" |> should equal (Equals(Identifier(None, "a"), String "1"))
...
let ``Parses simple JOINs`` () =
  parseFragment from "from t1, t2" |> should equal (Join(InnerJoin, Table("t1", None), Table("t2", None), Boolean true))
...
let ``Parse sub-query`` () =
  ...
  parseFragment from "FROM (SELECT col FROM table) t" |> should equal (SubQuery(query, "t"))
