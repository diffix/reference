...
When joining rows from two selectables, the combined rows AID instances is the union of the AID instances of the rows being joined. Crucially AID instances of the same kind (i.e. `email`) are treated as distinct AID instances! You might therefore end up with AID instances such as `[email-1[1]; email-2[1, 2]; email-3[1, 3]]` all referencing the same underlying `email` column, where each AID instance might contain partially or fully overlapping AID values (`1` in the example).

The same procedure is followed when joining sensitive and non-sensitive data as well. The only difference being that the non-sensitive data have no AID instances.

For a more in-depth discussion about why AID instance sets have to be merged, but not combined, have a look at the "Wide and Narrow" section of the [attack](attacks.md) document.


## Aggregating rows

When aggregating rows, the resulting aggregate has AID instances that are the union of the AID instances of the rows being aggregated. Each AID instance is taken the union of independently, that is to say that a row with two AID instances of kind `email` (say `email-1` and `email-2` resulting from a selectable having been joined with itself) after the aggregation has an `email-1` AID instance that is the union of all the `email-1` AID instances of the aggregated rows, and an `email-2` AID instance that is the union of all the `email-2` AID instances of the aggregated rows. Crucially `email-1` and `email-2` AID instances do not mix.
...
Since we have multiple AID instances per row this data either comes from a database table with
...
| card_type  | AID value sets                          | cnt1 (flattened) | cnt1 (real) |
| ---------- | --------------------------------------- | ---------------: | ----------: |
...
To derive `cnt2` we repeat the same procedure, taking the union of AID instances across the rows being joined. In particular we see that there are two occurrences of rows with a `cnt1` (the flattened kind) of 2. These will be aggregated to a single row and their respective AID instances merged.
...
| cnt1 | AID value sets                                | cnt2 (flattened) | cnt2 (real) |
| ---: | --------------------------------------------- | ---------------: | ----------: |
...
| cnt2 | AID value sets                                | cnt3 (flattened) | cnt3 (real) |
| ---: | --------------------------------------------- | ---------------: | ----------: |
