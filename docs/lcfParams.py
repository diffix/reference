...
def do_basic_probs(lower,mean_sd,counts,trials):
    results = {}
    results['(mean,sd)'] = []
    for count in counts:
    ...
    for i in range(len(mean_sd)):
        ...
        for count in counts:
            for _ in range(trials):
                ...
                suppress = do_suppress(count,0,upper,mean,sd)
                if not suppress:
                    results[count][i] += 1
    for count in counts:
...
def do_conditional_probs(lower,mean_sd,counts,trials):
    res = {}
    res['(mean,sd)'] = []
    res['N'] = []
    res['Prob N AIDs (sup)'] = []
    ...
    for i in range(len(mean_sd)):
        ...
        for n in range(1,int(mean)):
            ...
            if numReported:
                ...
                res['Prob reported'].append(f"{trials}++")
        res['(mean,sd)'].append(' ')
        res['N'].append(' ')
    ...
    print(f'''
Suppose that an attacker knows that there are either N or N+1 AIDs in a bucket.  Suppose that the probability of either outcome is 50%. The following table shows three things (lower={lower}):
...
# Let's measure the following mean values (plus lower)
base_means = [2,2.5,3,4,6,8]
# Let's measure a factor of this many SDs between lower and mean
sd_factors = [2,3,4,5]
# Let's measure these values for lower
lowers = [1.5,2.5]
...
for lower in lowers:
    ...
    for base_count in base_counts:
        counts.append(base_count + int(lower))
    means = []
    for base_mean in base_means:
        means.append(base_mean + int(lower))
    mean_sd = []
    for mean in means:
        for sd_factor in sd_factors:
            sd = round((mean - int(lower)) / sd_factor,1)
            mean_sd.append([mean,sd])
    do_basic_probs(lower,mean_sd,counts,trials)
    do_conditional_probs(lower,mean_sd,counts,trials)
